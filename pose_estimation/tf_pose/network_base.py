...
class BaseNetwork(object):
    ...
    @layer
    def normalize_resnet(self, input, name):
        input = tf.divide(input, 255.0, name=name + '_divide')
        input = tf.subtract(input, 0.5, name=name + '_subtract')
        input = tf.multiply(input, 2.0, name=name + '_multiply')
        return input
    ...
    def max_pool(self, input, k_h, k_w, s_h, s_w, name, padding=DEFAULT_PADDING):
        ...
        return tf.nn.max_pool(input,
                              ...
                              name=name)
    @layer
    def resblock(self, input,c_o,stride, name):
        x1=tf.keras.layers.Conv2D(c_o,kernel_size=(3,3),strides=stride,padding="same")(input)
        x1=tf.keras.layers.BatchNormalization()(x1)
        x1=tf.keras.layers.ReLU()(x1)
        x1=tf.keras.layers.Conv2D(c_o,kernel_size=(3,3),strides=1,padding="same")(x1)
        x1=tf.keras.layers.BatchNormalization()(x1)
        if stride!=1:
            x1=tf.keras.layers.Conv2D(c_o,kernel_size=(1,1),strides=stride,padding="valid")(x1)
            x1=tf.keras.layers.BatchNormalization()(x1)
        x1=tf.keras.layers.add([input,x1])
        x1=tf.keras.layers.ReLU()(x1)
        return x1
