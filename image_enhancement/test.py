...
parser.add_argument("--input", "-i", type=str, default='../IMG_0186/', help='q_t/')
parser.add_argument("--result", "-r", type=str, default='../results1', help='result folder')
...
parser.add_argument("--highpercent", "-hp", type=int, default=100, help='should be in [85,100], linear amplification')
...
imh = Network.build_imh((None, None, 3))
imh.load_weights('models3/161_.h5')
opt = keras.optimizers.Adam(lr=2 * 1e-04, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
imh.compile(loss='mse', optimizer=opt)
...
for i in range(len(path)):
    ...
    print(img_A_path)
    '''
    img_B =utls.imread_color("../gt/"+img_A_path.split("_")[0].split("/")[-1]+".png")
    img_B=cv2.resize(img_B,(256,256))
    '''
    starttime = time.clock()
    [_,_,out_pred] = imh.predict(img_A)
    endtime = time.clock()
    ...
    '''
    ssim = utls.psnr_cau(img_A, img_B)
    sum_ssim+=ssim
    '''
    utls.imwrite(img_name, outputs)
'''
print('------------------------------------------------')
print("[Epoch %d]  [SSIM_AVE :%f]" % (176,  sum_ssim/len(path)))
print('------------------------------------------------')
'''
