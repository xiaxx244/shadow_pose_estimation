...
def main():
    ...
    #if not os.path.exists('./results/unet_seg_weights.h5'):
    ...
    if not os.path.exists('./results/'):
        os.mkdir('./results')
    if os.path.exists('./weights/unet_seg_weights.h5'):
        model.load_weights('./weights/unet_seg_weights.h5')
        model.summary()
    save_file = './weights/{}_seg_weights.h5'.format(args.model) if nClasses == 2 \
        else './weights/{}_parse_weights.h5'.format(args.model)
    checkpoint = ModelCheckpoint(save_file, monitor='val_acc', save_best_only=True, save_weights_only=True, verbose=1, period=1)
